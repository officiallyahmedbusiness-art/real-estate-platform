...
export default async function AdminTeamUsersPage() {
  const { role } = await requireRole(["owner", "admin"], "/admin/team/users");
    ...
    <div className="min-h-screen bg-[var(--bg)] text-[var(--text)]">
      ...
      <main className="mx-auto w-full max-w-6xl space-y-8 px-6 py-10">
        <div className="flex flex-wrap items-center justify-between gap-3">
          <div>
            <h1 className="text-2xl font-semibold">{t("admin.hub.team.title")}</h1>
            <p className="text-sm text-[var(--muted)]">{t("admin.hub.team.subtitle")}</p>
          </div>
        ...
        <Section title={t("team.users.list.title")} subtitle={t("team.users.list.subtitle")}>
          <UserManagementList profiles={profiles} actorRole={role === "owner" ? "owner" : "admin"} />
        </Section>
      </main>
      <SiteFooter />
    </div>
