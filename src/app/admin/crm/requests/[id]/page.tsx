...
export default async function AdminCrmRequestDetailPage({
  params,
}: {
          ...
          <Card className="space-y-3">
            ...
            <div className="grid gap-2">
              {notes.length === 0 ? (
                ...
                notes.map((note) => (
                  <div key={note.id} className="rounded-xl border border-[var(--border)] bg-[var(--surface)] p-3 text-sm">
            ...
            {activity.length === 0 ? (
              ...
              activity.map((entry) => (
                <div key={entry.id} className="flex flex-wrap items-center justify-between gap-2 text-xs">
                  ...
                  <span className="text-[var(--muted)]">{new Date(entry.created_at).toLocaleString(locale)}</span>
                </div>
              ))
            )}
          </Card>
